...
$this->cssLink(common_path('js/css/tufte-graph.css'));
    ...
    function showScripts()
    {
        if (Event::handle('StartShowScripts', array($this))) {
            if (Event::handle('StartShowJQueryScripts', array($this))) {
                    ...
                    $this->script('graphs.js');
        	    $this->script('jquery.enumerable.js');
      	            $this->script('jquery.tufte-graph.js');
        	    $this->script('raphael.js');
    ...
    function showNoticeForm()
    {
        ...
        $tabs = array('status' => _m('TAB','My Profile'));
...
$this->showProfileBlock();
            $this->elementEnd('div');
$this->elementStart('div',array('id' =>'awesome-graph' , 'class'=>'graph', 'style'=>'width: 370px; height: 200px;'));
$this->elementEnd('div');
$user = common_current_user();
 $this->elementStart('input',array('type' => 'hidden', 'id' => 'user_id' ,'value' => $user->id));
$this->elementEnd('input');
 //$this->elementStart('input',array('type' => 'button', 'class' => 'button','name' => 'showprogress','id' => 'showprogress','value' => 'Show my progress','onclick' => 'loadProgressGraph("individual","' .  $user->id . '")'));
//$this->elementEnd('input');
            foreach ($tabs as $tag => $title) {
